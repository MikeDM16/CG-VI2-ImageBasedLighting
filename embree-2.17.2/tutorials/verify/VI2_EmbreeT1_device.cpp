...
#define LEVEL_FACTOR   64.0f
	...
	static Vec3fa depth_shade(RTCRay& ray);
	static Vec3fa diffuse_shade(RTCRay& ray);
	static Vec3fa direct_no_shadows_shade(RTCRay& ray);
	static Vec3fa direct_with_shadows_shade(RTCRay& ray, RayStats& stats);
	static Vec3fa Whitted_shade(RTCRay& ray, int depth, RayStats& stats);
	...
	Vec3fa renderPixelStandard(float x, float y, const ISPCCamera& camera, RayStats& stats)
	{
		...
		// for depth shading
		//color = depth_shade(ray);

		// for diffuse (Kd) shading
		//color = diffuse_shade(ray);

		// for direct no shadows shading
		//color = direct_no_shadows_shade(ray);

		// for direct with shadows shading
		//color = direct_with_shadows_shade(ray, stats);

		// Whitted shading
		color = Whitted_shade(ray, 1, stats);
	...
	// for Kd shading
	Vec3fa diffuse_shade(RTCRay& ray) {
		...
		Vec3fa color = Vec3fa(0.5f, 0.f, 0.f);
	...
	// direct_no_shadows shading
	Vec3fa direct_no_shadows_shade(RTCRay& ray) {
		...
		if (reduce_max(material->Kd) > 0.f) {
			for (size_t i = 0; i < g_ispc_scene->numLights; i++)
			{
				...
				Light_SampleRes ls = l->sample(l, dg, Vec2f(0.f));
	...
	// direct_with_shadows shading
	Vec3fa direct_with_shadows_shade(RTCRay& ray, RayStats& stats) {
	...
	// Whitted shading
#define MAX_DEPTH 3
	Vec3fa Whitted_shade(RTCRay& ray, int depth, RayStats& stats) {
		RTCRay spec_ray;
		...
		if ((depth < MAX_DEPTH) && (cosi >0.f)) {
			...
			if (reduce_max(material->Ks) > 0.f) {
				...
				RayStats_addRay(stats);
				Vec3fa L = Whitted_shade(spec_ray, depth + 1, stats);
